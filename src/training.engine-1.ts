...
function resolveProgressionLevel(
  ...
  let guard = 0;
  while (guard < 10 && athleteModel.exerciseMastery[curId]?.readyForProgression) {
    const node = PROGRESSION_TREE[curId];
    const nextId = node?.children?.find(child => EXERCISE_REGISTRY[child]);
    if (!nextId) break;
    curId = nextId;
    progressed = true;
    guard++;
...
// A prescription is a starting target, not a command to accumulate junk volume.
// The generator deliberately prefers rep progression over automatic set inflation.
const MAX_PLANNED_SETS = 2;
const MIN_PLANNED_SETS = 1;
const MAX_SESSION_MINUTES = 25;
const MIN_SESSION_MINUTES = 12;
const ALLOWED_EQUIPMENT = new Set(['']); // bodyweight-only by default
const MAX_REP_CAP = 20;
const MAX_HOLD_CAP = 60;

function isEquipmentCompatible(exId: string): boolean {
  const ex = EXERCISE_REGISTRY[exId];
  if (!ex) return false;
  return ex.equipment.every(item => ALLOWED_EQUIPMENT.has(item));
}

/** Resolve an exercise to the nearest usable regression without silently
 * claiming that an unavailable movement was performed. */
function resolveEquipmentSafe(exId: string): string | null {
  let curId: string | undefined = exId;
  const seen = new Set<string>();
  while (curId && !seen.has(curId)) {
    seen.add(curId);
    if (isEquipmentCompatible(curId)) return curId;
    curId = EXERCISE_REGISTRY[curId]?.regressionId;
  }
  return null;
}

function clampSets(rawSets: number, phase: MesocyclePhase, multiplier: number): number {
  if (phase === 'deload') return 1;
  // Never let phase/adaptation multipliers compound into 4–8-set prescriptions.
  return Math.max(MIN_PLANNED_SETS, Math.min(MAX_PLANNED_SETS, Math.round(rawSets * Math.min(multiplier, 1.25))));
}

function clampRepTarget(value: number, range: [number, number]): number {
  return Math.max(1, Math.min(MAX_REP_CAP, Math.round(Math.max(range[0], Math.min(range[1], value)))));
}

function clampHoldTarget(value: number, range: [number, number]): number {
  return Math.max(5, Math.min(MAX_HOLD_CAP, Math.round(Math.max(range[0], Math.min(range[1], value)))));
}
...
export function rawToPlanned(
  ...
  const progressed = resolveProgressionLevel(baseExId, athleteModel);
  const equipmentSafeId = resolveEquipmentSafe(progressed.id);
  if (!equipmentSafeId) return null;

  const exId = equipmentSafeId;
  const ex = EXERCISE_REGISTRY[exId];
  if (!ex) return null;
  const exerciseName = ex.name;
  const wasProgressed = progressed.progressed && exId !== baseExId;
  ...
  const combinedMult = Math.max(0.70, Math.min(1.20, globalMult * exMult));
  ...
  let sets = clampSets(raw.sets, phase, volMult * combinedMult);
  let type: 'reps' | 'timed' = ex.holdRange ? 'timed' : raw.type;
  let targetReps: number | 'max' = raw.reps;

  if (type === 'timed') {
    const mid = ex.holdRange
      ? Math.round((ex.holdRange[0] + ex.holdRange[1]) / 2)
      : (typeof raw.reps === 'number' ? raw.reps : 30);
    const history = hasHistory && m!.averageDuration > 0 ? m!.averageDuration : mid;
    targetReps = clampHoldTarget(history * intMult * combinedMult, ex.holdRange ?? [10, 60]);
  } else if (raw.reps === 'max') {
    // "MAX" means a technical-capacity test, not an arbitrary 30-rep target.
    // On a new movement, use a conservative registry midpoint; thereafter use
    // the athlete's recent best/average rather than inventing volume.
    const history = hasHistory && m!.averageReps > 0 ? m!.averageReps : Math.round((ex.repRange[0] + ex.repRange[1]) / 2);
    targetReps = clampRepTarget(history * intMult * combinedMult, ex.repRange);
  } else {
    const historicalAvg = hasHistory && m!.averageReps > 0 ? m!.averageReps : raw.reps;
    targetReps = clampRepTarget(historicalAvg * intMult * combinedMult, ex.repRange);
  }

  if (aiAdj && !overridden) {
    if (aiAdj.decision === 'increase_reps' && targetReps !== 'max') {
      targetReps = type === 'timed'
        ? clampHoldTarget(Number(targetReps) + 5, ex.holdRange ?? [10, 60])
        : clampRepTarget(Number(targetReps) + 1, ex.repRange);
    } else if (aiAdj.decision === 'reduce_reps' && targetReps !== 'max') {
      targetReps = type === 'timed'
        ? clampHoldTarget(Number(targetReps) - 5, ex.holdRange ?? [10, 60])
        : clampRepTarget(Number(targetReps) - 1, ex.repRange);
    } else if (aiAdj.decision === 'increase_sets') {
      sets = Math.min(MAX_PLANNED_SETS, sets + 1);
    } else if (aiAdj.decision === 'reduce_sets') {
      sets = Math.max(MIN_PLANNED_SETS, sets - 1);
    }
  }
  ...
  return {
    ...
    adjustmentReason: !isEquipmentCompatible(baseExId)
      ? `Equipment-safe regression from ${EXERCISE_REGISTRY[baseExId]?.name ?? raw.name}`
      : wasProgressed
...
export function generateWorkout(
  ...
  // Convert to planned exercises with equipment, volume, and adaptation safety.
  // Null means the movement cannot be performed with the current default setup.
  let planned: PlannedExercise[] = raw.exercises
    .map(e => rawToPlanned(e, phase, adaptation, athleteModel, aiAdjMap))
    .filter((e): e is PlannedExercise => e !== null);

  const estimateMinutes = (items: PlannedExercise[]): number => {
    const workTime = items.reduce((a, e) => {
      const value = e.targetReps === 'max' ? 8 : (e.targetDuration ?? Number(e.targetReps));
      return a + e.sets * (e.type === 'timed' ? value : value * 2.5);
    }, 0);
    const restTime = items.reduce((a, e) => a + Math.max(30, e.sets - 1) * e.restSeconds, 0);
    return Math.round((workTime + restTime + 360) / 60);
  };

  // Whole-session validator: trim accessory work first, then set count, until
  // the workout fits the hard time budget. This is the final authority after
  // phase, fatigue, progression, weak-point and AI adjustments have all run.
  while (planned.length > 1 && estimateMinutes(planned) > MAX_SESSION_MINUTES) {
    planned = planned.slice(0, -1);
  }
  if (estimateMinutes(planned) > MAX_SESSION_MINUTES) {
    planned = planned.map(e => ({ ...e, sets: 1 }));
  }
  const estimatedDuration = Math.max(MIN_SESSION_MINUTES, Math.min(MAX_SESSION_MINUTES, estimateMinutes(planned)));
  ...
  const reasoning = adaptation?.deloadActive
    ...
    : isPost90
      ...
      : `Day ${training.programDay} — ${phase} phase — ${estimatedDuration} min, equipment-safe`;
